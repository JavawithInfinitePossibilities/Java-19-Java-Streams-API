...
import com.sid.tutorials.spring.boot3.java.app.Test.StreamAverage;
 ...
 */
@SpringBootTest(classes = Section24Averages.class)
class Section24AveragesTest {
    @Autowired
    private MockDataPrep mockDataPrep;

    @Autowired
    private StreamAverage streamAverage;
    ...
    void mappingtoDoublesExerciseSolutionTest() {
        try {
            System.out.println(streamAverage.getAverageCar());
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
